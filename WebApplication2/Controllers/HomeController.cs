...
namespace WebApplication2.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            var f = db.students.Where(x => x.phonenumber == L.Email).SingleOrDefault();
            var T = db.Teachers.Where(x => x.phonenumber == L.Email).SingleOrDefault();

            if (f!=null && f.phonenumber.Count()> 0)
            {
                TempData["Role"] = f.Role.RName.ToString();
            
                return RedirectToAction("StudentList");

            }
            if (T != null && T.phonenumber.Count() > 0)
            {
                
                TempData["Role"] = T.Role.RName.ToString();
                return RedirectToAction("StudentList");
        ...
        public ActionResult Reports(ReportsModel R)
        {
            ...
            if (R.AddmissionNum==0)
            {
                ...
                R.Marklist = db.Marks.Where(x => x.Admissionum == R.AddmissionNum).ToList();
            var t = 0;
                foreach (var n in R.Marklist)
                {
                    R.TotalMarks += n.submarks;
